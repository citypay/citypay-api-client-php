...
```php
paylinkTokenCloseRequest($token): \CityPay\Model\Acknowledgement
```
...
try {
    $result = $apiInstance->paylinkTokenCloseRequest($token);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling PaylinkApi->paylinkTokenCloseRequest: ', $e->getMessage(), PHP_EOL;
}
```
...
```php
tokenAttachmentStatus($token, $attachment): \CityPay\Model\Acknowledgement
```
...
try {
    $result = $apiInstance->tokenAttachmentStatus($token, $attachment);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling PaylinkApi->tokenAttachmentStatus: ', $e->getMessage(), PHP_EOL;
}
```
